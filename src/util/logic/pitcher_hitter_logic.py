...
class PitcherHitterLogic:
    ...
    def get_swing_outcome(
        self, pitch_x: float, pitch_y: float, hitability: float
    ) -> str:

        rand_pitch_hitability = hitability
        cur_hitability = self.determine_hitability_from_value(
            pitch_x, pitch_y, rand_pitch_hitability
        )

        return cur_hitability

    def determine_hitability_from_value(
        self,
        pitch_x: float,
        pitch_y: float,
        pitch_hitability: float,
        ump_outcome: str,
        rand_pitch_hitability: float = 0.02,
    ) -> str:
        ...
        if is_strike:
            if pitch_hitability > (rand_pitch_hitability + rand_chance):
                outcome = "Hit"
            else:  # bad outcome is it looking or swinging
                # Player Attribute for chase
                if rand_pitch_hitability % 2 == 0:

                    # Foul tip
                    if rand_pitch_hitability % 3 == 0:
                        outcome = "Fouled Strike"
                    else:
    ...
    def calculate_hitability_value(
        self, pitch_x: float, pitch_y: float, hit_odds_calculator=0.45
    ) -> float:
        ...
        gaussian_value = np.exp(
            -((rand_x_pitch - mean) ** 2 + (rand_y_pitch - mean) ** 2) / (2 * sig**2)
        )
    ...
    def is_strike(self, pitch_x: float, pitch_y: float, ump_odds=0.12) -> bool:
        ...
        if (
            pitch_x < x0_margin
            or pitch_x > x1_margin
            or pitch_y < y0_margin
            or pitch_y > y1_margin
        ):
            return False
        ...
        gaussian_value = np.exp(
            -((pitch_x - mean) ** 2 + (pitch_y - mean) ** 2) / (2 * sigma**2)
        )
